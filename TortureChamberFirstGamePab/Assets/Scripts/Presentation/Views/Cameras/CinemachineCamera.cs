...
namespace Scripts.Presentation.Views.Cameras
{
    public class CinemachineCamera : View
    {
        ...
        [SerializeField] private float _angle = 0.1f;
        ...
        private void UpdateRotation()
        {
            if (Input.GetKey(KeyCode.Z))
                transform.Rotate(0, -_angle * _rotateSpeed, 0);

            if (Input.GetKey(KeyCode.X))
                transform.Rotate(0, _angle * _rotateSpeed, 0);

            if (Input.GetKey(KeyCode.R))
                transform.Rotate(-_angle * _rotateSpeed, 0, 0);
